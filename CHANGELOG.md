...
## [6.0.0](https://github.com/LiquidLogicLabs/git-action-docker-metadata/compare/v5.10.1...v6.0.0) (2026-03-24)

### Features

* sync with upstream docker/metadata-action v6.0.0 — node24 runtime, ESM (`type: module`), `@actions/core` v3, Vitest test framework, ESLint v9 flat config, `nodenext` TypeScript module resolution
* `commentNoInfix` behavior: inline `#` characters in label/annotation/tag values are now preserved; only lines beginning with `#` are treated as comments

### No GitHub API dependencies

This release retains zero GitHub API or Octokit usage. Context is derived exclusively from git commands (`simple-git`) and workflow event payloads.
